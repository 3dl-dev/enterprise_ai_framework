...
async def my_spend(since: str | None = None, user: str = Depends(require_user)):
    ...
    mine: dict[str, dict] = {}
    # cached_requests, failed_requests and refused_requests ride along with the rest
    # (findings 10 and 41): a person looking at their own $0 is owed the same explanation
    # the operator gets, and carrying it in one rendering but not the other is how the two
    # views start disagreeing. The first two are subtotals OF `requests`, never subtracted
    # from it. refused_requests is NOT inside `requests` — the gateway never admitted
    # those, and a person told "you made 8 requests" when it refused 6 of them for being
    # over budget would read the bill as charging for the refusals. See metering._FAILED.
    total = {"requests": 0, "cached_requests": 0, "failed_requests": 0,
             "refused_requests": 0, "spend": 0.0,
    ...
    for r in rows:
        ...
        acc = mine.setdefault(surface, {"surface": surface, "requests": 0,
                                        "cached_requests": 0, "failed_requests": 0,
                                        "refused_requests": 0,
                                        "spend": 0.0,
                                        "prompt_tokens": 0, "completion_tokens": 0})
        for k in ("requests", "cached_requests", "failed_requests", "refused_requests",
...
async def admin_overview(since: str | None = None,
    ...
    for r in rows:
        ...
        acc = people.setdefault(who, {
            "username": who, "requests": 0, "cached_requests": 0,
            "failed_requests": 0, "refused_requests": 0, "spend": 0.0,
            "prompt_tokens": 0, "completion_tokens": 0, "surfaces": {},
        })
        surface = r.get("surface") or "(unknown)"
        s = acc["surfaces"].setdefault(
            surface,
            {"requests": 0, "cached_requests": 0, "failed_requests": 0,
             "refused_requests": 0, "spend": 0.0},
        )
        for k in ("requests", "cached_requests", "failed_requests", "refused_requests",
                  "spend", "prompt_tokens", "completion_tokens"):
    ...
    for owner in budgets:
        people.setdefault(owner, {"username": owner, "requests": 0, "cached_requests": 0,
                                  "failed_requests": 0, "refused_requests": 0,
